...
APP_VS_FILE_NAME := virtualservice.yaml
APP_CLUSTER_A_DIR := ./app/cluster-a
APP_CLUSTER_B_DIR := ./app/cluster-b
...
deploy-a:
	...
	kubectl apply -f <(./istioctl kube-inject --hub=${HUB} --tag=${TAG} -f ${APP_CLUSTER_A_DIR}/${APP_FILE_NAME} |\
		sed -e "s,${HUB}/proxy:,${HUB}/proxyv2:,g") --context=${CLUSTER_A}
	kubectl apply -f ${APP_CLUSTER_A_DIR}/${APP_VS_FILE_NAME} --context=${CLUSTER_A}
deploy-a.istio:
...
delete-a:
	kubectl delete -f ${APP_CLUSTER_A_DIR}/${APP_FILE_NAME} --context=${CLUSTER_A} || true
	kubectl delete -f ${APP_CLUSTER_A_DIR}/${APP_VS_FILE_NAME} --context=${CLUSTER_A} || true
delete-a.istio:
...
deploy-b:
	# TODO: set env var to use proxyv2 instead of sed
	kubectl apply -f <(./istioctl kube-inject --hub=${HUB} --tag=${TAG} -f ${APP_CLUSTER_B_DIR}/${APP_FILE_NAME} |\
		sed -e "s,${HUB}/proxy:,${HUB}/proxyv2:,g") --context=${CLUSTER_B}
	kubectl apply -f ${APP_CLUSTER_B_DIR}/${APP_VS_FILE_NAME} --context=${CLUSTER_B}
deploy-b.istio:
...
delete-b:
	kubectl delete -f ${APP_CLUSTER_B_DIR}/${APP_FILE_NAME} --context=${CLUSTER_B} || true
	kubectl delete -f ${APP_CLUSTER_B_DIR}/${APP_VS_FILE_NAME} --context=${CLUSTER_B} || true
delete-b.istio:
